...
class LoginForm(FlaskForm):
    username = StringField('Username',
                           [validators.Length(min=4, max=20, message="Username should be 4-20 characters long")])
...
class SignUpForm(FlaskForm):
    ...
    nickname = StringField('Nickname',
                           [validators.Length(min=1, max=20, message="Nickname should be 1-20 characters long")])
    email = StringField('Email Address',
                        [validators.Length(min=6, max=50, message="Email should be 6-50 characters long")])
    password = PasswordField('Password', [validators.DataRequired(),
    ...
    try:
        ...
        if request.method == "POST":
            ...
            response = usertable.query(
                ...
                KeyConditionExpression=Key('Nickname').eq(escape_string(nickname))
            )
            if response['Count'] > 0:
